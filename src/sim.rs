...
use crate::world::{MaterialId, CHUNK_SIZE, EMPTY};
...
#[cfg(feature = "legacy-world-sim")]
mod legacy_world_sim {
    use super::*;
    ...
    fn has_same_neighbor_in_radius(
        world: &World,
        p: [i32; 3],
        id: MaterialId,
        radius_sq: i32,
    ...
    fn try_absorb_water_for_tree_growth(
        world: &mut World,
        p: [i32; 3],
        rng: &mut XorShift32,
    ) -> bool {
    ...
    mod tests {
        use super::MATERIALS;
        use crate::material_ids::GENERATED_MATERIAL_IDS;

        #[test]
        fn generated_material_ids_match_material_table() {
            for &(name, id) in GENERATED_MATERIAL_IDS {
                assert_eq!(
                    MATERIALS[id as usize].id, id,
                    "generated ID for {name} drifted from MATERIALS table"
                );
            }
        }
    }

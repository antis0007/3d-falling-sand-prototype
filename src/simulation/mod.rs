...
#[derive(Default)]
struct SparseVoxelPages {
    pages: HashMap<[i32; 3], Vec<MaterialVoxel>>,
}

impl SparseVoxelPages {
    fn set(&mut self, coord: VoxelCoord, material_id: u16, density: f32) {
        let page_coord = [
    ...
    fn get(&self, coord: VoxelCoord) -> Option<MaterialVoxel> {
        let page_coord = [
            coord.x.div_euclid(BRICK_EDGE),
            coord.y.div_euclid(BRICK_EDGE),
            coord.z.div_euclid(BRICK_EDGE),
...
impl SimulationBackend for GpuFluidBackend {
    ...
    fn queue_place_edit(&mut self, coord: VoxelCoord, material_id: u16) {
        self.command_buffer.push(SimCommand::PlaceOrErase {
            coord,
            material_id,
